...
public class CustomElasticQuerySource : IQuerySource
{
    public const string SourceName = "CustomElasticsearch";
    public string Name => SourceName;

    private readonly ICustomElasticQueryService _queryService;
    private readonly ILiquidTemplateManager _liquidTemplateManager;
    private readonly ISession _session;
    private readonly JavaScriptEncoder _javaScriptEncoder;
    private readonly TemplateOptions _templateOptions;

    public CustomElasticQuerySource(ICustomElasticQueryService queryService,
    ...
    public async Task<IQueryResults> ExecuteQueryAsync(Query query, IDictionary<string, object> parameters)
    {
        ...
        else
        {
            ...
            var nonHitInfos = new List<KeyValuePair<string, JsonNode>>();

            var suggests = JsonSerializer.Serialize(docs.Suggests, JOptions.UnsafeRelaxedJsonEscaping);
            var keyValuePair = KeyValuePair.Create("suggest", (JsonNode)JsonValue.Create(suggests));

            nonHitInfos.Add(keyValuePair);

            results.Add(new JsonObject(nonHitInfos));
